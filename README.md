...
2. 用 Presentation Director 完成 intake 和 brief confirmation。
3. 交给 Codex `Presentations` 生成 v1，并完成 render QA、style-review、final compare。
4. 输出到 `PPTX/<task-slug>/final/`。
...
如果目标是专业、可编辑的 `.pptx`，优先按 [专业可编辑 PPTX 主流程](docs/pptx-master-workflow.md) 使用 Codex `Presentations` 插件。Claude Code / 本地代理的备用路径是 `skills/pptx` + pptxgenjs。Marp / VS Code 只保留给快速写作、预览、PDF 和放映版输出，不作为默认的专业 PPTX 路径。
